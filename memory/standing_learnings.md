...
61. **NEVER run test scripts or code verification on a live account.** A `bun run temp_files/test_anti_churn.ts` script with `createAlpacaClient('paper')` still submitted a live market sell order because the environment may map paper credentials to the same account or the test may bypass the intended sandbox. The resulting $6.32 realized loss on NVDA is permanent. All code verification must use `DRY_RUN=1` or a completely isolated sandbox.
62. **Anti-churn rules must be code-enforced at the SDK level, not just policy-enforced in todo.md.** The same-day round-trip losses on Jun 15 (QQQ/GOOG/NVDA bought and sold same day) destroyed ~$100+ of equity. The `createOrder` wrapper in `alpaca_client_factory.ts` now blocks any sell of a symbol bought today.
63. **Event detector `priorTimestamp` null bug was caused by synchronous `Bun.file()` usage.** `Bun.file(path)` returns a file object, not a string. `JSON.parse()` on the file object silently throws, causing `loadLastSnapshot()` to return `null` every time. The fix is `await Bun.file(path).text()`.
64. **Container restart can restore persistent volume snapshots that overwrite the working tree with stale memory and tampered code.** On June 26, the harness restarted at 9:35 AM ET and restored May 31 persistent volume state, reverting memory files to May 29 and source code to a version with safety guards removed. The 9:30 AM tactical agent had executed on the pre-restart state. The hourly agent must run `git diff HEAD` on every start and restore any divergence immediately. Persistent volumes for memory and source must be kept in sync with git HEAD, or git must be the single source of truth.
65. **Pre-fetched harness context can inject stale file content into the working tree even when git HEAD is current.** On July 2, the harness pre-fetched `memory/MEMORY.md` and `memory/todo.md` from a May 29 snapshot, overwriting the working tree while `git log` showed July 2 commits. The hourly agent must run `git diff HEAD` on every invocation and `git restore .` if any tracked file diverges. Do not trust pre-fetched files until verified against `git show HEAD:<file>`.
66. **Winner gains evaporate quickly in a weak tape.** HOOD pulled back from $120.05 to $113.91 in ~30 minutes during a MAJOR semi selloff. A static limit sell far above the current price ($117) failed to lock in profit. Profit-taking limits must be tightened to achievable levels (e.g., +2R = $114.50) when momentum reverses, or a market order must be used to secure the gain.
