# Tactical Todo — Updated 2026-07-07 14:38Z (Tuesday 10:38 AM ET)
*Updated by hourly strategist. Fresh directives for Tuesday Jul 7 regular session.*

## CRITICAL — Alpaca API Data Corruption
- `get-positions` returns `[]` since at least 10:21 AM ET.
- `get-account` shows `equity: $2,267.81`, `long_market_value: 0` — this is CORRUPTED.
- **Portfolio history confirms true equity ~$9,549.** True positions still exist in clearing records.
- **No filled orders today (Jul 7).**
- **Raw REST API bypass is PROHIBITED.** A test sell for AVGO was accepted as `sell_to_open` (short). It was canceled immediately. Do NOT attempt raw API bypasses.
- **True reconstructed book:** QQQ 6, VOO 2, AVGO 3, HOOD 5. Cash ~$2,268. True equity ~$9,536.
...
- **Current (live bar):** $705.80 (–1.49% unrealized est.)
...
- **Current (live bar):** $686.23 (+1.44% unrealized est.)
...
- **Current (live bar):** $364.00 (–2.14% unrealized est.)
- **Stop:** $365.00 (hard disaster stop) — **BREACHED.**
- **Profit target:** $420.00. Do NOT trim or sell below $420 without fresh hourly authorization.
- **Rationale:** AI chip alpha. Hard stop breached at $364.00. Chip sector under pressure from Samsung earnings sell-off. The only reason we still hold is API corruption blocking sells.
- **Concentration:** 11.5% of reconstructed equity. Under 15% cap.
- **CRITICAL INSTRUCTION:** If `get-positions` recovers and shows AVGO 3 shares BEFORE the next hourly run, IMMEDIATELY submit limit SELL 3 AVGO at best bid or limit $363.95. Do NOT wait for next hourly. Stop breach must be honored. SDK short-sale guard should permit this once positions API recovers.
...
- **Current (live bar):** $113.84 (+12.17% unrealized est., +2.5R)
...
- QQQ $158.82 + VOO $32.92 + AVGO $23.85 + HOOD $32.50 = $248.09 (2.60% of reconstructed equity). Under 6% daily loss cap.
...
- **Rationale:** Momentum leader with multiple positive catalysts. Pullback to $105 would be a –7.7% dip from current $113.84 and a test of prior resistance-turned-support. R/R = 2.0 ($20 gain to $125 target vs $10 risk to $95 stop). Concentration post-fill = ~11.6% of reconstructed equity (under 15% cap).
...
### RKLB — DO NOT RE-BUY
- **Reason:** Exited Jul 6 at $93.25. 24-hour cooldown applies until after 17:43Z Jul 7. DO NOT re-buy RKLB before then.
- **DO NOT BUY RKLB** today before 17:43Z.
...
## Next Hourly — Tuesday Jul 7 15:35Z (11:35 AM ET)
- HOLD QQQ, VOO, HOOD.
...
- **RKLB buy trigger remains DEACTIVATED** until after 17:43Z Jul 7.
...
- **Watch AVGO $365.** Stop level breached. Priority is API recovery so we can execute the stop.
