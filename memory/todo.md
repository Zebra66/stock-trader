...
- **Account equity / cash / buying power:** $9,795.91 / $8,406.91 / $34,802.80
...
- **Blocker preventing deployment:** QQQ and SOXX are above our sell prices ($690.79 and $552.30/$523.12); we cannot buy them without chasing. GOOG and AVGO are below sell prices ($370.50 and $405.16) but our limit orders are slightly below current market. If they do not fill by close, reassess Tuesday with higher triggers.

## Position Instructions — Monday Jun 15 (3:35 PM ET)

### VOO — HOLD 2 — DO NOT SELL OR ADD
- Weight 14.19%, below 20% non-QQQ ETF cap. Adding 1 share at current price (~$694.51) would breach cap (~21.2% of equity = code-rejected).
...
- Current price ~$743.02. Market up +3.01% today after +3.42% 1W. Do NOT chase.
...
- **Concentration cap check:** QQQ max 45% of equity = $4,408.16. Buying 2 shares at $689.50 = $1,379 = 14.1% of equity. Well within cap.
...
- Current price ~$624.66. Market up +4.76% today after +8.80% 1W. Do NOT chase.
...
- Down –6.66% 1M. No clear catalyst, but beaten-down mega-cap.
- **Trigger:** BUY **1 share** if price drops to **$368.50 or below**. Day limit order at **$368.50**.
  - **Target:** $400.00 (2.0R from entry). Let winners run.
...
- **Concentration cap check:** GOOG max 15% of equity = $1,469.39. Buying 1 share at $368.50 = 3.76% of equity. Well within cap.
...
- Fully exited June 4 at $405.16. Current price $393.39 — **below our prior sell price.**
- Post-earnings correction deepening. Down –8.16% 1M.
- **Trigger:** BUY **1 share** if price drops to **$391.50 or below**. Day limit order at **$391.50**.
  ...
  - **Rationale:** Risk = $20 (0.20% of equity). Well below 2% max per trade.
- **Concentration cap check:** AVGO max 15% of equity = $1,469.39. Buying 1 share at $391.50 = 4.00% of equity. Well within cap.
- **Do NOT buy AVGO above $405.16.** That is our prior sell price; buying above it is poor execution.

### RKLB — DO NOT BUY — DO NOT RE-BUY
- No prior position. Current price $106.89.
...
- Fully exited June 5 at $207.80. Current price $211.51. **Above our prior sell price.**
- No re-entry authorized. Buying above $207.80 is poor execution.
- Risk-off tape and Oracle AI spending fears (-12%) create spillover risk for AI/semis.

### META — DO NOT BUY
- `bannedSymbols: ["META"]` active in `memory/.trading_lock.json`. Code-level rejection if attempted.

### HOOD — DO NOT BUY
- Up +7.68% today, +17.26% 1W. No clear catalyst beyond crypto beta.
- Chasing a strong run. Avoid.

### GLD / EIS / SHLD / QTUM / ARKX — DO NOT BUY
- Risk-on tape. No edge. No catalysts. Thin liquidity.
- GLD trade count 13, volume 924 at 15:36Z — thin. Limit orders only, or defer.
...
- **SOXX:** Extremely thin (8 trades, 116 vol at 15:36Z). Limit orders only.
- **EIS / SHLD / QTUM / ARKX / GLD:** Very low trade count / volume. Limit orders only.

## Today's Deployment Queue
1. **GOOG 1 share @ $368.50 day limit** — valid if price pulls back to $368.50 before close.
2. **AVGO 1 share @ $391.50 day limit** — valid if price pulls back to $391.50 before close.
3. **QQQ 2 shares @ $689.50 GTC** — valid on any future pullback to $690 or below.
4. **SOXX 1 share @ $575.00 GTC** — valid on any future pullback to $575 or below.
5. **No new orders** for RKLB, NVDA, META, HOOD, GLD, EIS, SHLD, QTUM, ARKX.
...
## Next Hourly Preview — Tuesday June 16, 9:35 AM ET
- Reassess GOOG/AVGO fill status. If unfilled, consider higher limit orders or market orders on flat/gap-down open.
- Monitor QQQ and SOXX for pullback to $690 and $575.
- Evaluate RKLB re-entry if Nasdaq 100 inclusion thesis strengthens.
- Investigate event detector `priorTimestamp: null` bug and repo corruption root cause.
- If gross exposure remains below 30%, consider aggressive deployment on Tuesday to close the gap.

## Repo Integrity Note
- **CRITICAL BREACH RESTORED this cycle.** Working tree files reverted to May 29 state. Source code guards removed. All tracked files staged-as-deleted. `.gitignore` missing.
- **Restored:** `git checkout -- .` from HEAD `64bf5c5`. `.gitignore` manually written and verified. Working tree clean.
- **Verified:** `git check-ignore node_modules/zod/package.json` returns exit 0.
- **Root cause unknown:** Tactical agent or harness process may have overwritten working tree with stale pre-fetched content. **Priority investigation.**
