# Tactical Todo For Next Execution

## Book-Level Instructions
- [ ] Use **account equity ($9,980.82)** as the sizing base. Do **not** size from raw margin buying power.
- [ ] Current live positions: **SOXX 2 shares**, **QTUM 2 shares**, and **RKLB 1 share**.
- [ ] Current gross exposure is about **12.7%**. Cap gross exposure around **25%** for this hour unless the next hourly review explicitly raises it.
- [ ] Keep at least **75% cash** after any new trades unless the tape materially improves.
- [ ] Treat the current book as already having semiconductor / AI exposure. Prefer **GOOG** and **QQQ** before adding more correlated chip risk.
- [ ] If multiple setups trigger, prioritize in this order: **GOOG -> QQQ -> AVGO -> NVDA -> RKLB**.
- [ ] Skip any trade where the expected move to first trim is not clearly at least **3x** estimated total round-trip friction.
- [ ] Estimated friction reminder: liquid mega-cap / ETF names can still cost roughly **5-20 bps** round trip once spread, slippage, CAT, SEC, and FINRA sell-side fees are included; thinner names can cost materially more.
- [ ] Use marketable limit orders when spreads are not very tight.
- [ ] **Do not average down** the existing SOXX or QTUM positions.
- [ ] Adds must be smaller than or equal to prior adds and only into winners.

## Existing Positions
- [ ] **HOLD SOXX** — current size is already near the desired **~10%** target allocation. **Do not add** this hour unless the next hourly cycle explicitly raises the target. **SELL SOXX** if price breaks below **457.5** to cut risk. **TRIM 1 share** above **467.5** if strength extends.
- [ ] **HOLD QTUM** — current size is already near the desired **~3%** target allocation for this account. **Do not add** unless price breaks above **136.8** with QQQ and SOXX both firm. **SELL QTUM** if price breaks below **134.2** to cut risk. **TRIM 1 share** above **138.8** if strength extends.
- [ ] **HOLD RKLB** — keep this as a tiny satellite only. **SELL RKLB** below **77.4** to cut risk. **TRIM 1 share** above **82.9** if strength extends.

## Active Buy / Sell Conditions For New Capital
- [ ] **BUY GOOG** if price reclaims **381.5-382.0** and holds, or if it breaks above **384.5** with confirmation — earnings quality and cloud/AI follow-through remain best-in-class. Start with **1 share** (~3.8% of equity). **SELL / REDUCE GOOG** above **389.5** to trim, or below **375.0** to cut risk.
- [ ] **BUY QQQ** if price reclaims **673.8-674.2** and holds, or if it breaks above **676.5** with confirmation — broad growth leadership remains stronger than broad-market beta. Start with **1 share** (~6.7% of equity). **SELL / REDUCE QQQ** above **681.5** to trim, or below **667.5** to cut risk.
- [ ] **BUY AVGO** only if price reclaims **415.0-416.0** and semis are firm, or if it breaks above **419.5** with confirmation — keep it smaller than SOXX because the sector is already represented. Start with **1 share** (~4.1% of equity). **SELL / REDUCE AVGO** above **424.5** to trim, or below **408.5** to cut risk.
- [ ] **BUY NVDA** only if price reclaims **198.5-199.5** with SOXX strong, or if it breaks above **202.0** cleanly — still a leader, but currently lower priority than GOOG / QQQ. Start with **1 share** (~2.0% of equity). **SELL / REDUCE NVDA** above **205.5** to trim, or below **194.0** to cut risk.

## Avoid / Low-Priority Names This Hour
- [ ] **AVOID META** unless price reclaims **618** and relative strength materially improves.
- [ ] **AVOID HOOD** unless price reclaims **78.0** and holds after the weak earnings / analyst-reset backdrop.
- [ ] **AVOID EIS** unless price clears **134.5** on tight spreads **and** a fresh live quote confirms the move; current broker quote is stale.
- [ ] **AVOID SHLD** unless price reclaims **69.5** with better relative strength.
- [ ] **AVOID GLD** this hour unless a clear risk-off shock appears; current account size makes the intended hedge sizing awkward.
- [ ] **AVOID VOO** for new capital while QQQ keeps superior relative strength.
- [ ] **AVOID ARKX** unless space momentum broadens materially beyond RKLB.

## Tactical Behavior Reminders
- [ ] If the tape becomes disorderly, do nothing and preserve cash.
- [ ] If GOOG / QQQ do not confirm, accept the current small exposure and wait.
- [ ] If SOXX, QTUM, or RKLB hit their stop levels, reduce first and reassess before opening replacement positions.
- [ ] If both GOOG and QQQ trigger cleanly, it is acceptable to take both; otherwise prefer the cleaner setup rather than forcing exposure.

## 2026-05-04 12:50 ET Hourly Refresh Notes
- [x] Live Alpaca account confirms the book is **SOXX 2 shares**, **QTUM 2 shares**, and **RKLB 1 share**.
- [x] Account snapshot at 12:49 ET: **$9,980.82 equity / $8,708.26 cash / $18,610.53 buying power**.
- [x] Current unrealized P&L is **-$19.18** overall (**SOXX -$15.10, QTUM -$3.17, RKLB -$0.91**).
- [x] Re-checked FMP and confirmed historical performance remains unavailable because **`FMP_API_KEY` is missing**.
- [x] Multi-timeframe performance was refreshed using the **Yahoo Finance chart API fallback**.
- [x] Fresh prices and nearby technical levels were refreshed for **GOOG / QQQ / AVGO / NVDA / SOXX / QTUM / RKLB**.
- [x] Best-add watchlist remains **GOOG / QQQ**, with **AVGO** tertiary and **NVDA** secondary only on stronger confirmation.
- [x] No prompt or code changes were made in this cycle.

## 2026-05-05 Tactical Pass
- [x] Fresh quote refresh via Yahoo Finance fallback showed: **AVGO 413.63, EIS 132.735, GLD 416.04, GOOG 381.10, HOOD 77.31, META 611.87, NVDA 197.665, QQQ 672.97, QTUM 135.89, RKLB 80.83, SHLD 68.395, SOXX 463.206, VOO 660.33, ARKX 32.685**.
- [x] **No trade conditions triggered**: GOOG / QQQ / AVGO / NVDA remained below their reclaim zones.
- [x] **Existing positions remained in hold zones**: SOXX held above stop and below trim, QTUM held above stop and below add/trim, RKLB held above stop and below trim.
- [x] Alpaca account/positions endpoints returned **ECONNREFUSED**, so no broker-side order was attempted this cycle.
